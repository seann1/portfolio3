...
(function($){
    $.fn.glitch = function(options) {
        var s = $.extend({
            bg: "#bada55",    // background color
            maxint: 15,     // max interval between glitchings
            minint: 2,      // min interval between glitchings
            maxglitch: 5,   // max number of twitches
            hshift: 20,      // max horizontal shift 
            vshift: 20,      // max vertical shift
            direction: 'random' // 'horizontal', 'vertical' or 'random'
        }, options);

        return this.each(function(){
            $t=$(this);
            $t.wrap('<span style="display:inline;position:relative">');
            ...
            var g = function(){
                ...
                for(i=0;i<rnd(s.maxglitch)+1;i++){
                    setTimeout(function(){
                        var css, dir;
                        if(s.direction=='random') dir=Math.random()<0.3?'horizontal':'vertical';
                        else dir=s.direction;
                        if(dir=='vertical') css={top: rnd(s.vshift+4), width: rnd(Math.floor(width*0.8))+2, height:height};
...
(function($){
    $.fn.glitch2 = function(options) {

        var newColor = ["#ffffff", "#bbffff", "#960000", "#00ff0c"];

        var s = $.extend({
            bg: newColor[Math.floor(Math.random() * newColor.length)],    // background color
            maxint: 4,     // max interval between glitchings
            minint: 2,      // min interval between glitchings
            maxglitch: 40,   // max number of twitches
